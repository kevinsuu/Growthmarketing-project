...
class LineMessageService:
    ...
    def send_flex_message_with_narrowcast(self, user_id, flex_message=None):
        """使用 Narrowcast API 發送 Flex Message 並追蹤狀態"""
        try:
            if flex_message is None:
                flex_message = self.create_flex_message()

            # 建立受眾群組
            group_description = f"User_{user_id}_{datetime.now().strftime('%Y%m%d%H%M%S')}"
            audience_group_id = self.create_audience_group(group_description)
            
            if not audience_group_id:
                return {
                    'success': False,
                    'message': '建立受眾群組失敗'
                }
            ...
            # 將用戶加入受眾群組
            if not self.add_audience(audience_group_id, user_id):
                return {
                    'success': False,
                    'message': '添加用戶到受眾群組失敗'
                }
    ...
    def get_narrowcast_status(self, request_id):
        """查詢 Narrowcast 訊息狀態"""
        try:
            url = f"https://api.line.me/v2/bot/message/progress/narrowcast?requestId={request_id}"
    ...
    def get_audience_group_status(self, audience_group_id):
        """查詢受眾群組狀態"""
        try:
